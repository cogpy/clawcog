...
metadata:
  { "opencog": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/opencog.sock"
SESSION=opencog-python
...
- Use `OPENCOG_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$OPENCOG_TMUX_SOCKET_DIR/opencog.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `OPENCOG_TMUX_SOCKET_DIR`).
