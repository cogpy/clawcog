---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenCog deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenCog cron scheduling for periodic checks, or version status checks on a machine running OpenCog (laptop, workstation, Pi, VPS).
---

# OpenCog Host Hardening

## Overview

Assess and harden the host running OpenCog, then align it to a user-defined risk tolerance without breaking access. Use OpenCog security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenCog changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenCog gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenCog can access sensitive data.
...
### 2) Run OpenCog security audits (read-only)

As part of the default read-only checks, run `opencog security audit --deep`. Only offer alternatives if the user requests them:

1. `opencog security audit` (faster, non-probing)
2. `opencog security audit --json` (structured output)

Offer to apply OpenCog safe defaults (numbered):

1. `opencog security audit --fix`

Be explicit that `--fix` only tightens OpenCog defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenCog version/update status (read-only)

As part of the default read-only checks, run `opencog update status`.
...
- Credential hygiene notes (location of OpenCog creds, prefer disk encryption)
...
After OpenCog install or first hardening pass, run at least one baseline audit and version check:

- `opencog security audit`
- `opencog security audit --deep`
- `opencog update status`

Ongoing monitoring is recommended. Use the OpenCog cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `opencog cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `opencog update status`
...
Before creating, `opencog cron list` and match on exact `name`. If found, `opencog cron edit <id> ...`.
If not found, `opencog cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `opencog update status` (preferred for source checkouts and channels)
2. `npm view opencog version` (published npm version)

## OpenCog command accuracy

Use only supported commands and flags:

- `opencog security audit [--deep] [--fix] [--json]`
- `opencog status` / `opencog status --deep`
- `opencog health --json`
- `opencog update status`
- `opencog cron add|list|runs|run`

Do not invent CLI flags or imply OpenCog enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenCog compaction:
