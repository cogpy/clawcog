...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OpenCogProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpenCogProtocol.AnyCodable] = [
            "presence": .init([OpenCogProtocol.AnyCodable(entry)]),
        ]
        let payload = OpenCogProtocol.AnyCodable(payloadMap)
