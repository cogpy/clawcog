...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.opencog", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OpenCogPeekabooBridgeServices?
    private static var opencogSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("OpenCog", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OpenCogPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.opencogSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.opencogSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OpenCogPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.opencog.peekaboo")
