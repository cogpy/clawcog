...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: OpenCogOAuthStore.AnthropicOAuthStatus = OpenCogOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: OpenCogOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = OpenCogOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = OpenCogOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try OpenCogOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. OpenCog can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: OpenCogOAuthStore.AnthropicOAuthStatus,
