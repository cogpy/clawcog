...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.opencog", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenCogSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenCogSessionPreviewEntry,
    ...
    private static func cache(payload: OpenCogSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenCogChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenCogKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenCogKit.AnyCodable]) -> [OpenCogChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenCogChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenCogChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenCogChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenCogChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenCogChatMessage) -> String? {
