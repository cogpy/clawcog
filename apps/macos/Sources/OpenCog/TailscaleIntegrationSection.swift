...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.opencog/opencog.json. Prefer OPENCOG_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.opencog/opencog.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.opencog/opencog.json. Restart the gateway to apply."
        }
