...
private let webChatSwiftLogger = Logger(subsystem: "ai.opencog", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OpenCogChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpenCogChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OpenCogChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpenCogChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OpenCogGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpenCogChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpenCogChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpenCogChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OpenCogChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpenCogChatView>) -> NSViewController
    {
