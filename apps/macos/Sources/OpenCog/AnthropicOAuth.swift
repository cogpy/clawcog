...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (OpenCog token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: OpenCogOAuthStore.AnthropicOAuthStatus = OpenCogOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.opencog", category: "anthropic-oauth")
...
enum OpenCogOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let opencogOAuthDirEnv = "OPENCOG_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "OpenCog OAuth token file not found"
            case .unreadableFile: "OpenCog OAuth token file not readable"
            case .invalidJSON: "OpenCog OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in OpenCog OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "OpenCog OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.opencogOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".opencog", isDirectory: true)
