...
enum DebugActions {
    private static let verboseDefaultsKey = "opencog.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "opencog.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenCogPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenCog", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenCogPaths.configURL
        guard
