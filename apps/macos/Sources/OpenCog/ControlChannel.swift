...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenCogProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.opencog", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenCogKit.AnyCodable]()) {
                $0[$1.key] = OpenCogKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenCogProtocol.AnyCodable?) -> [String: OpenCogProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenCogProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenCogKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenCogProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("opencog.control.heartbeat")
    static let controlAgentEvent = Notification.Name("opencog.control.agent")
}
