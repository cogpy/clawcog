...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENCOG_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/opencog")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("opencog-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("opencog-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/opencog/ (rolling or stdout), or nil if none exist.
