...
enum OpenCogLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = OpenCogOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenCogFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenCogLogging.bootstrapIfNeeded()
        let label = OpenCogLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenCogOSLogHandler: LogHandler {
...
struct OpenCogFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenCogLogging.parseLabel(self.label)
