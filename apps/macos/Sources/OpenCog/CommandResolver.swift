...
enum CommandResolver {
    private static let projectRootDefaultsKey = "opencog.gatewayProjectRootPath"
    private static let helperName = "opencog"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let opencogEntry = root.appendingPathComponent("opencog.mjs").path
        if FileManager().isReadableFile(atPath: opencogEntry) { return opencogEntry }
        let binEntry = root.appendingPathComponent("bin/opencog.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/opencog")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let opencogPaths = self.opencogManagedPaths(home: home)
        if !opencogPaths.isEmpty {
            extras.insert(contentsOf: opencogPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + opencogPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func opencogManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".opencog"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func opencogExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOpenCogExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("opencog.mjs").path,
            root.appendingPathComponent("bin/opencog.js").path,
        ]
    ...
    static func hasAnyOpenCogInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.opencogExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func opencogNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let opencogPath = self.projectOpenCogExecutable(projectRoot: root) {
                return [opencogPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "opencog", subcommand] + extraArgs
            }
            if let opencogPath = self.opencogExecutable(searchPaths: searchPaths) {
                return [opencogPath, subcommand] + extraArgs
            }

            let missingEntry = """
            opencog entrypoint missing (looked for dist/index.js or opencog.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real opencog CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/opencog"
        ...
        \(cliSection)
        \(projectSection)
        if command -v opencog >/dev/null 2>&1; then
          CLI="$(command -v opencog)"
          opencog \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/opencog.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/opencog.mjs"
            node "$PRJ/opencog.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/opencog.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/opencog.js"
            node "$PRJ/bin/opencog.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent opencog"
          pnpm --silent opencog \(quotedArgs);
        else
          echo "opencog CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenCogConfigFile.loadDict()
