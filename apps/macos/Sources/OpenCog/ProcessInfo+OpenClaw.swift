...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.opencog.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["OPENCOG_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "opencog.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "opencog.nixMode") { return true }
