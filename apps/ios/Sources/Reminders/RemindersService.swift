...
final class RemindersService: RemindersServicing {
    func list(params: OpenCogRemindersListParams) async throws -> OpenCogRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpenCogReminderPayload], Error>) in
        ...
        return OpenCogRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenCogRemindersAddParams) async throws -> OpenCogRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenCogReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenCogRemindersAddPayload(reminder: payload)
    }
