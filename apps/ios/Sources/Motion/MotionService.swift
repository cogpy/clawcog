...
final class MotionService: MotionServicing {
    func activities(params: OpenCogMotionActivityParams) async throws -> OpenCogMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpenCogMotionActivityEntry], Error>) in
        ...
        return OpenCogMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpenCogPedometerParams) async throws -> OpenCogPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<OpenCogPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpenCogPedometerPayload(
