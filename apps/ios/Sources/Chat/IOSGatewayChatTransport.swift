...
struct IOSGatewayChatTransport: OpenCogChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpenCogChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenCogChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenCogChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenCogChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenCogChatAttachmentPayload]) async throws -> OpenCogChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenCogChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(OpenCogChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(OpenCogGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenCogGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenCogChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenCogAgentEventPayload.self)
                        {
