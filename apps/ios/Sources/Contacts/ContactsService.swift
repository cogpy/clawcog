...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpenCogContactsSearchParams) async throws -> OpenCogContactsSearchPayload {
        ...
        return OpenCogContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpenCogContactsAddParams) async throws -> OpenCogContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpenCogContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpenCogContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpenCogContactPayload {
        OpenCogContactPayload(
