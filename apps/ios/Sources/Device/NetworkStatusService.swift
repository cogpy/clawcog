...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> OpenCogNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> OpenCogNetworkStatusPayload {
        let status: OpenCogNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> OpenCogNetworkStatusPayload {
        OpenCogNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
