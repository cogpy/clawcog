...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = OpenCogLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = OpenCogLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.opencog.ios", category: "GatewaySettings")
    ...
    var body: some View {
        NavigationStack {
                ...
                Section("Device") {
                    DisclosureGroup("Features") {
                        ...
                        Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                            Text("Off").tag(OpenCogLocationMode.off.rawValue)
                            Text("While Using").tag(OpenCogLocationMode.whileUsing.rawValue)
                            Text("Always").tag(OpenCogLocationMode.always.rawValue)
                        }
                        ...
                        Text("Keeps the screen awake while OpenCog is open.")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
            ...
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                ...
                guard let mode = OpenCogLocationMode(rawValue: newValue) else { return }
                Task {
    ...
    private var locationMode: OpenCogLocationMode {
        OpenCogLocationMode(rawValue: self.locationEnabledModeRaw) ?? .off
    }
