...
private final class TestChatTransport: @unchecked Sendable, OpenCogChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenCogChatHistoryPayload]
    private let sessionsResponses: [OpenCogChatSessionsListResponse]

    private let stream: AsyncStream<OpenCogChatTransportEvent>
    private let continuation: AsyncStream<OpenCogChatTransportEvent>.Continuation

    init(
        historyResponses: [OpenCogChatHistoryPayload],
        sessionsResponses: [OpenCogChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OpenCogChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenCogChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenCogChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [OpenCogChatAttachmentPayload]) async throws -> OpenCogChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OpenCogChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenCogChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenCogChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpenCogChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OpenCogChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OpenCogChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OpenCogChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OpenCogChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OpenCogChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OpenCogChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OpenCogChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
