...
private let chatUILogger = Logger(subsystem: "ai.opencog", category: "OpenCogChatUI")

@MainActor
@Observable
public final class OpenCogChatViewModel {
    public private(set) var messages: [OpenCogChatMessage] = []
    ...
    public var attachments: [OpenCogPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpenCogChatPendingToolCall] = []
    public private(set) var sessions: [OpenCogChatSessionEntry] = []
    private let transport: any OpenCogChatTransport
    ...
    private var pendingToolCallsById: [String: OpenCogChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OpenCogChatTransport) {
    ...
    public var sessionChoices: [OpenCogChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [OpenCogChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [OpenCogChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpenCogPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenCogChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpenCogChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [OpenCogChatMessage]) -> [OpenCogChatMessage] {
        var result: [OpenCogChatMessage] = []
    ...
    private static func dedupeKey(for message: OpenCogChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpenCogChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> OpenCogChatAttachmentPayload in
            OpenCogChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpenCogChatSessionEntry {
        OpenCogChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OpenCogChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpenCogChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpenCogAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpenCogChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpenCogPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
