...
@MainActor
struct OpenCogChatComposer: View {
    @Bindable var viewModel: OpenCogChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OpenCogChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OpenCogChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OpenCogPendingAttachment.id)
                { (att: OpenCogPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OpenCogPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenCogChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OpenCogChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OpenCogChatTheme.subtleCard)
        .clipShape(Capsule())
    }
