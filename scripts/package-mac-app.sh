...
# Build and bundle OpenCog into a minimal .app we can open.
# Outputs to dist/OpenCog.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenCog.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenCog"
BUNDLE_ID="${BUNDLE_ID:-ai.opencog.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/opencog/opencog/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenCog/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenCogBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenCogGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenCog"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenCog"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenCog"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenCog" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenCog/Resources/OpenCog.icns" "$APP_ROOT/Contents/Resources/OpenCog.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenCog/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenCogKit resources"
OPENCOGKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenCogKit_OpenCogKit.bundle"
if [ -d "$OPENCOGKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenCogKit_OpenCogKit.bundle"
  cp -R "$OPENCOGKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenCogKit_OpenCogKit.bundle"
else
  echo "WARN: OpenCogKit resource bundle not found at $OPENCOGKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenCog"
killall -q OpenCog 2>/dev/null || true
