...
IMAGE_NAME="opencog-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="opencog-net-e2e-$$"
GW_NAME="opencog-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPENCOG_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPENCOG_SKIP_CHANNELS=1" \
	  -e "OPENCOG_SKIP_GMAIL_WATCHER=1" \
	  -e "OPENCOG_SKIP_CRON=1" \
	  -e "OPENCOG_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
