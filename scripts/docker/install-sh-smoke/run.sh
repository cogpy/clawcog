...
INSTALL_URL="${OPENCOG_INSTALL_URL:-https://opencog.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENCOG_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENCOG_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="opencog"
PACKAGE_NAME="${OPENCOG_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENCOG_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENCOG_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENCOG_INSTALL_LATEST_OUT:-}"
fi
