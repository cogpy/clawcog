...
ZIP=${1:?"Usage: $0 OpenCog-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/opencog/opencog/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OpenCog-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/opencog/opencog/releases/download/v${VERSION}/"}
