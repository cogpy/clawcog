...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/opencog/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenCog Re-Auth" -c "After getting key, run: ssh $SERVER '~/opencog/scripts/mobile-reauth.sh'" --id opencog-auth
        ;;
    *)
