#!/bin/bash
# Setup OpenCog Auth Management System
...
echo "=== OpenCog Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'opencog-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenCog message: Send warning via OpenCog itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/opencog-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/opencog-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/opencog-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now opencog-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status opencog-auth-monitor.timer"
echo ""
echo "Quick commands:"
