 ...
 * `opencog update`. Use it to verify the completion installation flow
...
\`opencog update\`, \`opencog doctor\`, and \`opencog onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenCog Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`opencog completion --install\` later to enable.`));
    return;
  }
