...
#
# Suggested cron: */30 * * * * /home/admin/opencog/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenCog notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., opencog-alerts)
...
STATE_FILE="$HOME/.opencog/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenCog if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use opencog
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenCog to $NOTIFY_PHONE..."
            opencog send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenCog is down. Run: ssh l36 '~/opencog/scripts/mobile-reauth.sh'" "urgent"
    exit 1
