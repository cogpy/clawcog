...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "opencog",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/opencog.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("opencog");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "opencog");
