...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".opencog");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENCOG_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENCOG_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENCOG_STATE_DIR.
