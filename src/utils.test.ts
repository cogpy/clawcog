...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "opencog-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "opencog-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "opencog-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "opencog-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.opencog when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "opencog-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENCOG_HOME over HOME", () => {
    vi.stubEnv("OPENCOG_HOME", "/srv/opencog-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/opencog-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENCOG_HOME prefix when OPENCOG_HOME is set", () => {
    vi.stubEnv("OPENCOG_HOME", "/srv/opencog-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/opencog-home")}/.opencog/opencog.json`)).toBe(
      "$OPENCOG_HOME/.opencog/opencog.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENCOG_HOME replacement when OPENCOG_HOME is set", () => {
    vi.stubEnv("OPENCOG_HOME", "/srv/opencog-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/opencog-home")}/.opencog/opencog.json`),
    ).toBe("config: $OPENCOG_HOME/.opencog/opencog.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/opencog")).toBe(path.resolve(os.homedir(), "opencog"));
  });
  ...
  it("prefers OPENCOG_HOME for tilde expansion", () => {
    vi.stubEnv("OPENCOG_HOME", "/srv/opencog-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/opencog")).toBe(path.resolve("/srv/opencog-home", "opencog"));
