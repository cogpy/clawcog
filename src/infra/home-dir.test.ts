...
describe("resolveEffectiveHomeDir", () => {
  it("prefers OPENCOG_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands OPENCOG_HOME when set to ~", () => {
    const env = {
      OPENCOG_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for OPENCOG_HOME", () => {
    const result = resolveRequiredHomeDir(
      { OPENCOG_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when OPENCOG_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ OPENCOG_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { OPENCOG_HOME: "/srv/opencog-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/opencog-home")}/x`);
  });
