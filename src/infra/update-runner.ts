...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "opencog.mjs");
    ...
    const doctorStep = await runStep(
      step("opencog doctor", doctorArgv, gitRoot, { OPENCOG_UPDATE_IN_PROGRESS: "1" }),
    );
