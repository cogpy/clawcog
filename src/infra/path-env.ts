...
function candidateBinDirs(opts: EnsureOpenCogPathOpts): string[] {
  ...
  // Bundled macOS app: `opencog` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "opencog");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/opencog` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "opencog"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `opencog` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenCogCliOnPath(opts: EnsureOpenCogPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENCOG_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OPENCOG_PATH_BOOTSTRAPPED = "1";
