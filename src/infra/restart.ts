...
export function triggerOpenCogRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENCOG_SYSTEMD_UNIT,
        process.env.OPENCOG_PROFILE,
      );
  ...
  const label =
    process.env.OPENCOG_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENCOG_PROFILE);
