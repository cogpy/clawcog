...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENCOG_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*opencog-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "opencog";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "opencog");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "opencog-gw");
  ...
  records.push(`_opencog-gw._tcp IN PTR ${instanceLabel}._opencog-gw._tcp`);
  records.push(`${instanceLabel}._opencog-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._opencog-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; opencog-content-hash: ${contentHash}\n${contentBody}`;
}
