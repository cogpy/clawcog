...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.OPENCOG_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.OPENCOG_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.OPENCOG_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[opencog] ${parsed.error}`);
    process.exit(2);
  }
