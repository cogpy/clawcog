...
import type { OpenCogConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: OpenCogConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenCogConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenCogConfig),
...
export function getCommandDetection(_cfg?: OpenCogConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenCogConfig) {
