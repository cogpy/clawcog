...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-commands-"));
...
function buildParams(commandBody: string, cfg: OpenCogConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
