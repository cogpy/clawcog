...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "opencog.sandbox=1");
  args.push("--label", `opencog.sessionKey=${params.scopeKey}`);
  args.push("--label", `opencog.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `opencog.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("opencog.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`opencog sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`opencog sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("opencog sandbox recreate --all");
}
