...
import { createOpenCogCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("opencog-ws-", async (workspaceDir) => {
      await withTempDir("opencog-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenCogCodingTools({ workspaceDir });
    ...
    await withTempDir("opencog-ws-", async (workspaceDir) => {
      await withTempDir("opencog-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello opencog");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("opencog-ws-", async (workspaceDir) => {
      const tools = createOpenCogCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("opencog-ws-", async (workspaceDir) => {
      await withTempDir("opencog-override-", async (overrideDir) => {
        const tools = createOpenCogCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("opencog-sandbox-", async (sandboxDir) => {
      await withTempDir("opencog-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "opencog-sbx-test",
        ...
        const tools = createOpenCogCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
