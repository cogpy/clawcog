...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): OpenCogConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-workspace-"));
    try {
