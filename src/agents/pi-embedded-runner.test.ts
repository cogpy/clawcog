...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-embedded-agent-"));
...
const ensureModels = (cfg: OpenCogConfig) => ensureOpenCogModelsJson(cfg, agentDir) as unknown;
