...
type WebSearchConfig = NonNullable<OpenCogConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpenCogConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("opencog configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.opencog.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://opencog.ai",
      "X-Title": "OpenCog Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
