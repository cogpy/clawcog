...
/**
 * Cleanup suspended OpenCog CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpenCogConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-cli-images-"));
  const paths: string[] = [];
