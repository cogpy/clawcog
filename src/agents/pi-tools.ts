...
function resolveExecConfig(cfg: OpenCogConfig | undefined) {
...
export function createOpenCogCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createOpenCogReadTool(freshReadTool)];
    }
