...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "opencog-models-" });
}

const MODELS_CONFIG: OpenCogConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureOpenCogModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureOpenCogModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureOpenCogModelsJson } = await import("./models-config.js");
      const { resolveOpenCogAgentDir } = await import("./agent-paths.js");

      await ensureOpenCogModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveOpenCogAgentDir(), "models.json");
