...
import { __testing, createOpenCogCodingTools } from "./pi-tools.js";
...
describe("createOpenCogCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpenCogTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenCogCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "opencog-outside.txt");
