...
function setSlackDmPolicy(cfg: OpenCogConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OpenCog";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OpenCog`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: OpenCogConfig, allowFrom: string[]): OpenCogConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
