...
export function resolveHooksConfig(cfg: OpenCogConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-opencog-token"] === "string"
      ? req.headers["x-opencog-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
