...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OPENCOG_SKIP_CHANNELS;
    prevSkipGmail = process.env.OPENCOG_SKIP_GMAIL_WATCHER;
    process.env.OPENCOG_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OPENCOG_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OPENCOG_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENCOG_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
