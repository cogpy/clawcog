...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.OPENCOG_CONFIG_PATH;
  const prevDisableCache = process.env.OPENCOG_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "opencog-canvas-auth-test-"));
  const configPath = path.join(dir, "opencog.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.OPENCOG_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.OPENCOG_DISABLE_CONFIG_CACHE;
    } else {
      process.env.OPENCOG_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
