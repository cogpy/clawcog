...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "opencog-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".opencog", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenCogConfig> = {},
): OpenCogConfig {
  const base: OpenCogConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "opencog"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".opencog", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
