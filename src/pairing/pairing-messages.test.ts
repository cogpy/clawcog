...
describe("buildPairingReply", () => {
  ...
  for (const testCase of cases) {
    it(`formats pairing reply for ${testCase.channel}`, () => {
      ...
      // CLI commands should respect OPENCOG_PROFILE when set (most tests run with isolated profile)
      const commandRe = new RegExp(
        `(?:opencog|opencog) --profile isolated pairing approve ${testCase.channel} <code>`,
      );
