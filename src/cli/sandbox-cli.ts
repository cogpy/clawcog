...
const SANDBOX_EXAMPLES = {
  main: [
    ["opencog sandbox list", "List all sandbox containers."],
    ["opencog sandbox list --browser", "List only browser containers."],
    ["opencog sandbox recreate --all", "Recreate all containers."],
    ["opencog sandbox recreate --session main", "Recreate a specific session."],
    ["opencog sandbox recreate --agent mybot", "Recreate agent containers."],
    ["opencog sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["opencog sandbox list", "List all sandbox containers."],
    ["opencog sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["opencog sandbox recreate --all", "Recreate all containers."],
    ["opencog sandbox recreate --session main", "Recreate a specific session."],
    ["opencog sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["opencog sandbox recreate --browser --all", "Recreate only browser containers."],
    ["opencog sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["opencog sandbox explain", "Show effective sandbox config."],
    ["opencog sandbox explain --session agent:main:main", "Explain a specific session."],
    ["opencog sandbox explain --agent work", "Explain an agent sandbox."],
    ["opencog sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.opencog.ai/cli/sandbox")}\n`,
    )
    .action(() => {
