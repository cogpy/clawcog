...
import { resolveOpenCogPackageRoot } from "../infra/opencog-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "opencog doctor entry": "Checking doctor entrypoint",
  "opencog doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "opencog";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const OPENCOG_REPO_URL = "https://github.com/opencog/opencog.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("opencog@")) {
    return trimmed.slice("opencog@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "opencog.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("opencog completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.OPENCOG_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENCOG_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENCOG_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENCOG_GIT_DIR to an empty folder or an opencog checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENCOG_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveOpenCogPackageRoot({
  ...
  defaultRuntime.log(theme.heading("OpenCog update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating OpenCog..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this OpenCog install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("opencog doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("opencog gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g opencog@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g opencog@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OPENCOG_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OPENCOG_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("opencog gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("opencog doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("opencog gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("opencog gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `opencog update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPENCOG_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENCOG_GIT_DIR to an empty folder or an opencog checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via OPENCOG_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OpenCog to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["opencog update", "Update a source checkout (git)"],
        ["opencog update --channel beta", "Switch to beta channel (git + npm)"],
        ["opencog update --channel dev", "Switch to dev channel (git + npm)"],
        ["opencog update --tag beta", "One-off update to a dist-tag or version"],
        ["opencog update --no-restart", "Update without restarting the service"],
        ["opencog update --json", "Output result as JSON"],
        ["opencog update --yes", "Non-interactive (accept downgrade prompts)"],
        ["opencog update wizard", "Interactive update wizard"],
        ["opencog --update", "Shorthand for opencog update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run opencog update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.opencog.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opencog update status", "Show channel + version status."],
          ["opencog update status --json", "JSON output."],
          ["opencog update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.opencog.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
