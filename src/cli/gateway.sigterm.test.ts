...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opencog-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENCOG_NO_RESPAWN: "1",
      OPENCOG_STATE_DIR: stateDir,
      OPENCOG_SKIP_CHANNELS: "1",
      OPENCOG_SKIP_GMAIL_WATCHER: "1",
      OPENCOG_SKIP_CRON: "1",
      OPENCOG_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENCOG_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "opencog-entry-bootstrap.mjs");
