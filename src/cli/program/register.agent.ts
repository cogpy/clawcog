...
${formatHelpExamples([
  ['opencog agent --to +15555550123 --message "status update"', "Start a new session."],
  ['opencog agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'opencog agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'opencog agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['opencog agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.opencog.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.opencog.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "opencog agents set-identity --identity-file ~/.opencog/workspace/IDENTITY.md --agent main",
