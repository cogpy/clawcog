...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.OPENCOG_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "opencog");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.OPENCOG_STATE_DIR = stateDir;
    process.env.OPENCOG_CONFIG_PATH = path.join(stateDir, "opencog.json");
