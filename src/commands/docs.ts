...
const SEARCH_TOOL = "https://docs.opencog.ai/mcp.SearchOpenCog";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.opencog.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('opencog docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.opencog.ai/");
      runtime.log(`Search: ${formatCliCommand('opencog docs "your query"')}`);
    }
    return;
  }
