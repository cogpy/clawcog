...
import { readConfigFileSnapshot, type OpenCogConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OpenCogConfig, model: string): OpenCogConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("opencog plugins install")}\`.`,
    );
  }
