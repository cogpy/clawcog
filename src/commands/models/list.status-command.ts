...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenCogAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("opencog models auth setup-token")}\` or \`${formatCliCommand("opencog configure")}\`.`
          : `Run \`${formatCliCommand("opencog configure")}\` or set an API key env var.`;
