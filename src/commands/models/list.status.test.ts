...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenCogAgentDir: vi.fn().mockReturnValue("/tmp/opencog-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/opencog-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/opencog-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenCogAgentDir: mocks.resolveOpenCogAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenCogAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/opencog-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/opencog-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/opencog-agent-custom");
