...
export async function updateConfig(
  mutator: (cfg: OpenCogConfig) => OpenCogConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenCogConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenCogConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("opencog agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
