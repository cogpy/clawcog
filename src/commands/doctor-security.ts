...
import type { OpenCogConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenCogConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("opencog security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("opencog configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("opencog config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("opencog doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("opencog config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
