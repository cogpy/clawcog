...
function resolveMode(cfg: OpenCogConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenCog doctor");

  const root = await resolveOpenCogPackageRoot({
  ...
  let cfg: OpenCogConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("opencog configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("opencog config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("opencog setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("opencog doctor --fix")}" to apply changes.`);
  }
