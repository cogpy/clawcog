...
beforeEach(() => {
  ...
  resolveOpenCogPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENCOG_STATE_DIR;
  originalUpdateInProgress = process.env.OPENCOG_UPDATE_IN_PROGRESS;
  process.env.OPENCOG_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opencog-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENCOG_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENCOG_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENCOG_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOpenCogPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenCogPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/opencog-root.js", () => ({
  resolveOpenCogPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "opencog-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPENCOG_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENCOG_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/opencog.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPENCOG_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
