...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenCog status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("opencog security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("opencog security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.opencog.ai/faq");
  runtime.log("Troubleshooting: https://docs.opencog.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("opencog status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("opencog logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("opencog status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("opencog gateway probe")}`);
  }
}
