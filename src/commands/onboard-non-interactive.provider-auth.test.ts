...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.OPENCOG_GATEWAY_PASSWORD,
    disableConfigCache: process.env.OPENCOG_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("OPENCOG_STATE_DIR", prev.stateDir);
  restoreEnvVar("OPENCOG_CONFIG_PATH", prev.configPath);
  restoreEnvVar("OPENCOG_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("OPENCOG_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("OPENCOG_SKIP_CRON", prev.skipCron);
  restoreEnvVar("OPENCOG_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("OPENCOG_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("OPENCOG_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("OPENCOG_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.OPENCOG_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "opencog.json");
  process.env.HOME = tempHome;
  process.env.OPENCOG_STATE_DIR = tempHome;
  process.env.OPENCOG_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("opencog-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("opencog-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("opencog-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("opencog-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("opencog-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("opencog-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
