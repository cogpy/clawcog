...
export function listAgentEntries(cfg: OpenCogConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpenCogConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenCogConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenCogConfig): AgentSummary[] {
