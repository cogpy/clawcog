...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENCOG_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENCOG_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENCOG_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENCOG_GATEWAY_PASSWORD");
  });
