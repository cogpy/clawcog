...
const requireAgentDir = () => {
  const agentDir = process.env.OPENCOG_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENCOG_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  const previousAgentDir = process.env.OPENCOG_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.OPENCOG_AGENT_DIR;
    } else {
      process.env.OPENCOG_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OPENCOG_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPENCOG_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-oauth-"));
    process.env.OPENCOG_STATE_DIR = tempStateDir;
    process.env.OPENCOG_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENCOG_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENCOG_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-minimax-"));
    process.env.OPENCOG_STATE_DIR = tempStateDir;
    process.env.OPENCOG_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENCOG_AGENT_DIR;
