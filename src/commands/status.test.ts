...
beforeAll(() => {
  previousProfile = process.env.OPENCOG_PROFILE;
...
vi.mock("../infra/opencog-root.js", () => ({
  resolveOpenCogPackageRoot: vi.fn().mockResolvedValue("/tmp/opencog"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/opencog/pnpm-lock.yaml",
      markerPath: "/tmp/opencog/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("OpenCog status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("opencog status --all") ||
          l.includes("opencog --profile isolated status --all") ||
          l.includes("opencog status --all") ||
          l.includes("opencog --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENCOG_GATEWAY_TOKEN = "abcd1234";
    try {
