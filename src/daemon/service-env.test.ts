...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENCOG_GATEWAY_PORT).toBe("18789");
    expect(env.OPENCOG_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENCOG_SERVICE_MARKER).toBe("opencog");
    expect(env.OPENCOG_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENCOG_SERVICE_VERSION).toBe("string");
    expect(env.OPENCOG_SYSTEMD_UNIT).toBe("opencog-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENCOG_LAUNCHD_LABEL).toBe("ai.opencog.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENCOG_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENCOG_SYSTEMD_UNIT).toBe("opencog-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENCOG_LAUNCHD_LABEL).toBe("ai.opencog.work");
    }
  });
});
