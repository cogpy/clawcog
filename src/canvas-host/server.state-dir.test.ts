...
describe("canvas host state dir defaults", () => {
  ...
  it("uses OPENCOG_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("OpenCog Canvas");
    } finally {
      await handler.close();
