...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("opencogCanvasA2UIAction");
    expect(out).toContain("opencogSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("opencogSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.opencogA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__opencog__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("opencog-a2ui-host");
      expect(html).toContain("opencogCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__opencog__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("opencogA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
