...
`;
    ...
    expect(parsed.opencog.emoji).toBe("💾");
    expect(parsed.opencog.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.opencog.emoji).toBe("📝");
    expect(parsed.opencog.events).toEqual(["command"]);
    expect(parsed.opencog.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.opencog.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"opencog": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"opencog": {"events": ["test"]}}');
  });
...
describe("resolveOpenCogMetadata", () => {
  it("extracts opencog metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenCogMetadata(frontmatter);
  ...
  it("returns undefined when opencog key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        opencog: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenCog" },
            { id: "npm", kind: "npm", package: "@opencog/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@opencog/hook");
  });
    ...
    "opencog":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with OpenCog" }],
      },
  }
---
...
`;
    ...
    const opencog = resolveOpenCogMetadata(frontmatter);
    expect(opencog).toBeDefined();
    expect(opencog?.emoji).toBe("💾");
    expect(opencog?.events).toEqual(["command:new"]);
    expect(opencog?.requires?.config).toEqual(["workspace.dir"]);
    expect(opencog?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const opencog = resolveOpenCogMetadata(frontmatter);
    expect(opencog?.emoji).toBe("disk");
    expect(opencog?.events).toEqual(["command:new"]);
  });
});
