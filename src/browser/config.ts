import type { BrowserConfig, BrowserProfileConfig, OpenCogConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENCOG_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "opencog" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_OPENCOG_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "opencog" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENCOG_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENCOG_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenCog browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for opencog/opencog CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenCogConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENCOG_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "opencog";
