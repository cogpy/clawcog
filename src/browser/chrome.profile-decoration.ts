...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".opencog-profile-decorated");
}
 ...
 */
export function decorateOpenCogProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENCOG_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENCOG_BROWSER_COLOR).toUpperCase();
