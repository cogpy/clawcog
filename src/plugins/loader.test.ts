...
import { loadOpenCogPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OPENCOG_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `opencog-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "opencog.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OPENCOG_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOpenCogPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OPENCOG_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadOpenCogPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOpenCogPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
