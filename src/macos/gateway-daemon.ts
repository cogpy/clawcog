...
declare const __OPENCOG_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __OPENCOG_VERSION__ === "string" && __OPENCOG_VERSION__) ||
  process.env.OPENCOG_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `opencog --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.OPENCOG_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.OPENCOG_GATEWAY_BIND ??
