...
import type { OpenCogConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareOpenCogVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "OPENCOG_GATEWAY_TOKEN",
  "OPENCOG_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): OpenCogConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenCogConfig;
}
...
function stampConfigVersion(cfg: OpenCogConfig): OpenCogConfig {
...
function warnIfConfigFromFuture(cfg: OpenCogConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenCogVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenCog (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OpenCogConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenCogConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OpenCogConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenCogConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenCogConfig) {
    clearConfigCache();
...
// module scope. `OPENCOG_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENCOG_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENCOG_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OpenCogConfig {
...
export async function writeConfigFile(cfg: OpenCogConfig): Promise<void> {
  clearConfigCache();
