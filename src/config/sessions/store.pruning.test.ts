...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real opencog.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.OPENCOG_SESSION_CACHE_TTL_MS;
    process.env.OPENCOG_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.OPENCOG_SESSION_CACHE_TTL_MS;
    } else {
      process.env.OPENCOG_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
