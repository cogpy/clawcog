...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.opencog/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          OPENCOG_STATE_DIR: path.join(home, ".opencog"),
          CLAWDBOT_STATE_DIR: undefined,
          OPENCOG_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          OPENCOG_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
