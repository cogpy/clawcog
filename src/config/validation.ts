...
import type { OpenCogConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenCogConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OpenCogConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenCogSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenCogConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenCogConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OpenCogConfig)),
    ),
  };
}
