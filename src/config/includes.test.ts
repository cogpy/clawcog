...
const ETC_OPENCOG_DIR = path.join(ROOT_DIR, "etc", "opencog");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "opencog.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOpenCogPath(...parts: string[]) {
  return path.join(ETC_OPENCOG_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOpenCogPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "opencog.json"))).toEqual({
      shared: true,
    });
  });
});
