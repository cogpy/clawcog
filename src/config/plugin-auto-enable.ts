import type { OpenCogConfig } from "./config.js";
...
function isTelegramConfigured(cfg: OpenCogConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: OpenCogConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: OpenCogConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: OpenCogConfig): boolean {
...
function isIMessageConfigured(cfg: OpenCogConfig): boolean {
...
function isWhatsAppConfigured(cfg: OpenCogConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpenCogConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: OpenCogConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpenCogConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: OpenCogConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: OpenCogConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: OpenCogConfig, pluginId: string): OpenCogConfig {
...
function registerPluginEntry(cfg: OpenCogConfig, pluginId: string): OpenCogConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
