...
service via the external `opencog` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OPENCOG_PROFILE`; legacy `com.opencog.*` is supported).
