---
summary: "OpenCog on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenCog on a Raspberry Pi
  - Running OpenCog on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenCog on Raspberry Pi

## Goal

Run a persistent, always-on OpenCog Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenCog features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled opencog

# Enable if not
sudo systemctl enable opencog

# Start on boot
sudo systemctl start opencog
```
...
```bash
# Check logs
journalctl -u opencog --no-pager -n 100

# Common fix: rebuild
cd ~/opencog  # if using hackable install
npm run build
sudo systemctl restart opencog
```
