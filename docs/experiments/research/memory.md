...
read_when:
  - Designing workspace memory (~/.opencog/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenCog integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.opencog/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.opencog/workspace` as canonical human-readable memory.
...
```
~/.opencog/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenCog**, but keep a separable core library.

### Why integrate into OpenCog?

- OpenCog already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `opencog memory recall "…" --k 25 --since 30d`
  - `opencog memory reflect --since 7d`
...
Pragmatic take for `~/.opencog/workspace`:
