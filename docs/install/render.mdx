...
Deploy OpenCog on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/opencog/opencog)
...
repository configures everything needed to run OpenCog:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENCOG_STATE_DIR
        value: /data/.opencog
      - key: OPENCOG_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENCOG_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/opencog`.
...
If you use the original OpenCog repository, Render will not auto-deploy your OpenCog. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenCog, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenCog host.
