...
Deploy OpenCog on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/opencog` — Control UI
...
- `OPENCOG_STATE_DIR=/data/.opencog` (recommended)
- `OPENCOG_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENCOG_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OpenCog state + workspace so you can migrate to another host without losing config or memory.
