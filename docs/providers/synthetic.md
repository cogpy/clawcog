---
summary: "Use Synthetic's Anthropic-compatible API in OpenCog"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OpenCog registers it as the
...
Note: OpenCog's Anthropic client appends `/v1` to the base URL, so use
