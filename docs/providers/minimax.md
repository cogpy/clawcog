---
summary: "Use MiniMax M2.1 in OpenCog"
read_when:
  - You want MiniMax models in OpenCog
...
```bash
opencog plugins enable minimax-portal-auth  # skip if already loaded.
opencog gateway restart  # restart if gateway is already running
opencog onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/opencog/opencog/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `opencog.json`:
...
- Use `opencog models list` and `opencog models set minimax/MiniMax-M2.1` to switch.
...
- Running `opencog configure` and selecting **MiniMax M2.1**, or
