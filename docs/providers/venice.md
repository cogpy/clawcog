---
summary: "Use Venice AI privacy-focused models in OpenCog"
read_when:
  - You want privacy-focused inference in OpenCog
...
```bash
opencog onboard --auth-choice venice-api-key
```
...
```bash
opencog chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, OpenCog shows all available Venice models. Pick based on your needs:
...
```bash
opencog models set venice/claude-opus-45
opencog models set venice/llama-3.3-70b
```

List all available models:

```bash
opencog models list | grep venice
```

## Configure via `opencog configure`

1. Run `opencog configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenCog automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
opencog chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
opencog chat --model venice/claude-opus-45

# Use uncensored model
opencog chat --model venice/venice-uncensored

# Use vision model with image
opencog chat --model venice/qwen3-vl-235b-a22b

# Use coding model
opencog chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `opencog models list` to see currently available models. Some models may be temporarily offline.
