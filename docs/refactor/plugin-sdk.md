...
- Publish as `opencog/plugin-sdk` (or export from core under `opencog/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenCogPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenCogConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenCogConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenCogConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenCogConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenCogConfig): string;
  };
};
```
...
- Introduce `opencog/plugin-sdk`.
- Add `api.runtime` to `OpenCogPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `opencogRuntime: ">=2026.2.0"`).
