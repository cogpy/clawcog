...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.opencog/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.opencog/node.json` (node id + pairing token).
...
Path: `~/.opencog/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.opencog/exec-approvals.sock",
...
- Unix socket at `~/.opencog/exec-approvals.sock` (0600).
