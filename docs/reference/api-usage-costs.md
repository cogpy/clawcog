...
OpenCog features that can generate provider usage or paid API calls.
...
- `opencog status --usage` and `opencog channels list` show provider **usage windows**
...
OpenCog can pick up credentials from:
...
- `opencog status --usage`
- `opencog models status --json`
...
`opencog models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
