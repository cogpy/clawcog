...
OpenCog integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenCog owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenCog spawns `imsg rpc` as a child process (legacy iMessage integration).
