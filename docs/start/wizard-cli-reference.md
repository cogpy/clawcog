...
read_when:
  - You need detailed behavior for opencog onboard
...
This page is the full reference for `opencog onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.opencog/opencog.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `opencog doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `opencog pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `opencog health`.
    - `opencog status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.opencog/.env` so launchd can read it.
...
- OAuth credentials: `~/.opencog/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.opencog/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`opencog onboard`](/cli/onboard)
