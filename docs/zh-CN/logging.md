...
你可以在 `~/.opencog/opencog.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/opencog.log"
  }
}
```
...
```bash
opencog channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.opencog/opencog.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/opencog/opencog-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- OpenCog 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
OPENCOG_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `opencog plugins enable diagnostics-otel` 启用插件。
...
- `opencog.tokens`（计数器，属性：`opencog.token`、`opencog.channel`、`opencog.provider`、`opencog.model`）
- `opencog.cost.usd`（计数器，属性：`opencog.channel`、`opencog.provider`、`opencog.model`）
- `opencog.run.duration_ms`（直方图，属性：`opencog.channel`、`opencog.provider`、`opencog.model`）
- `opencog.context.tokens`（直方图，属性：`opencog.context`、`opencog.channel`、`opencog.provider`、`opencog.model`）

消息流：

- `opencog.webhook.received`（计数器，属性：`opencog.channel`、`opencog.webhook`）
- `opencog.webhook.error`（计数器，属性：`opencog.channel`、`opencog.webhook`）
- `opencog.webhook.duration_ms`（直方图，属性：`opencog.channel`、`opencog.webhook`）
- `opencog.message.queued`（计数器，属性：`opencog.channel`、`opencog.source`）
- `opencog.message.processed`（计数器，属性：`opencog.channel`、`opencog.outcome`）
- `opencog.message.duration_ms`（直方图，属性：`opencog.channel`、`opencog.outcome`）

队列 + 会话：

- `opencog.queue.lane.enqueue`（计数器，属性：`opencog.lane`）
- `opencog.queue.lane.dequeue`（计数器，属性：`opencog.lane`）
- `opencog.queue.depth`（直方图，属性：`opencog.lane` 或 `opencog.channel=heartbeat`）
- `opencog.queue.wait_ms`（直方图，属性：`opencog.lane`）
- `opencog.session.state`（计数器，属性：`opencog.state`、`opencog.reason`）
- `opencog.session.stuck`（计数器，属性：`opencog.state`）
- `opencog.session.stuck_age_ms`（直方图，属性：`opencog.state`）
- `opencog.run.attempt`（计数器，属性：`opencog.attempt`）

### 导出的 span（名称 + 关键属性）

- `opencog.model.usage`
  - `opencog.channel`、`opencog.provider`、`opencog.model`
  - `opencog.sessionKey`、`opencog.sessionId`
  - `opencog.tokens.*`（input/output/cache_read/cache_write/total）
- `opencog.webhook.processed`
  - `opencog.channel`、`opencog.webhook`、`opencog.chatId`
- `opencog.webhook.error`
  - `opencog.channel`、`opencog.webhook`、`opencog.chatId`、`opencog.error`
- `opencog.message.processed`
  - `opencog.channel`、`opencog.outcome`、`opencog.chatId`、`opencog.messageId`、`opencog.sessionKey`、`opencog.sessionId`、`opencog.reason`
- `opencog.session.stuck`
  - `opencog.state`、`opencog.ageMs`、`opencog.queueDepth`、`opencog.sessionKey`、`opencog.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `opencog doctor`。
