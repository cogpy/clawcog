---
summary: "Chrome extension: let OpenCog drive your existing Chrome tab"
read_when:
...
The OpenCog Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate opencog-managed Chrome profile.
...
OpenCog then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenCog release (npm package) as static files. There is no separate “build” step.

After upgrading OpenCog:

- Re-run `opencog browser extension install` to refresh the installed files under your OpenCog state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OpenCog ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want OpenCog to control.
...
- `ON`: attached; OpenCog can drive that tab.
...
Debugging: `opencog sandbox explain`
...
`opencog browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `opencog browser extension install` first to copy the extension to a stable location under your OpenCog state directory.
...
- **This is not isolated** like the dedicated opencog-managed profile.
