...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `opencog sandbox explain`.
...
Legacy `agent.*` configs are migrated by `opencog doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=opencog-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENCOG_STATE_DIR:-$HOME/.opencog}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
