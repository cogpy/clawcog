---
summary: "Direct `opencog agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `opencog agent` (direct agent runs)

`opencog agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `opencog message --target`).
...
```bash
opencog agent --to +15555550123 --message "status update"
...
opencog agent --to +15555550123 --message "Summon reply" --deliver
