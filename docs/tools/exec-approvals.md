...
- **gateway host** → `opencog` process on the gateway machine
...
`~/.opencog/exec-approvals.json` directly.

CLI: `opencog approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
