---
summary: "Agent tool surface for OpenCog (browser, canvas, nodes, message, cron) replacing legacy `opencog-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `opencog-*` skills
title: "Tools"
---

# Tools (OpenCog)

OpenCog exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `opencog-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `opencog.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenCog logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`opencog node run`).
...
- Requires a Brave API key (recommended: `opencog configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenCog-managed browser.
...
- Quick smoke: `opencog nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `opencog gateway` restart in-place)
