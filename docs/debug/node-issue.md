...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenCog
...
Running OpenCog via Node with `tsx` fails at startup with:

```
[opencog] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `opencog status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch opencog.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node opencog.mjs status` works on Node 25.
