...
- During heartbeat runs, OpenCog treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `opencog sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenCog skips the heartbeat run entirely (no model call).
...
```bash
opencog system event --text "Check for urgent follow-ups" --mode now
```
