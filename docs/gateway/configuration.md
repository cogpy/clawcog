---
summary: "All configuration options for ~/.opencog/opencog.json with examples"
read_when:
...
OpenCog reads an optional **JSON5** config from `~/.opencog/opencog.json` (comments + trailing commas allowed).

If the file is missing, OpenCog uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.opencog/workspace`). You usually only need a config to:
...
OpenCog only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `opencog doctor`, `opencog logs`, `opencog health`, `opencog status`, `opencog service`, `opencog help`).
- Run `opencog doctor` to see the exact issues.
- Run `opencog doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `opencog config set`. Keep a backup of `~/.opencog/opencog.json`.
...
```bash
opencog gateway call config.get --params '{}' # capture payload.hash
opencog gateway call config.apply --params '{
...
```bash
opencog gateway call config.get --params '{}' # capture payload.hash
opencog gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.opencog/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@opencog", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.opencog/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.opencog/workspace" }],
}
```
...
{ "$include": "/etc/opencog/base.json5" } // absolute
...
```json5
// ~/.opencog/clients/mueller/agents.json5
[
...
```json5
// ~/.opencog/clients/mueller/broadcast.json5
{
...
OpenCog reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, OpenCog runs your login shell and imports only the missing expected keys (never overrides).
...
- `OPENCOG_LOAD_SHELL_ENV=1`
- `OPENCOG_SHELL_ENV_TIMEOUT_MS=15000`
...
OpenCog stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.opencog/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.opencog/credentials/oauth.json` (or `$OPENCOG_STATE_DIR/credentials/oauth.json`)
...
- `~/.opencog/agent/*` (migrated by `opencog doctor` into `~/.opencog/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `OPENCOG_OAUTH_DIR`
- Agent dir (default agent root override): `OPENCOG_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, OpenCog imports `oauth.json` entries into `auth-profiles.json`.
...
If set, OpenCog derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/opencog/opencog-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/opencog/opencog.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.opencog/credentials/whatsapp/biz
          // authDir: "~/.opencog/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `opencog doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@opencog", "opencog"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@opencog"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.opencog/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.opencog/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.opencog/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.opencog/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.opencog/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.opencog/workspace-home" },
      { id: "work", workspace: "~/.opencog/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `opencog.json`).
...
OpenCog starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["opencog-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-opencog",
...
OpenCog starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `opencog plugins install @opencog/mattermost` (or `./extensions/mattermost` from a git checkout).
...
OpenCog starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
OpenCog spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, OpenCog
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/opencog" } },
}
```
...
When a file exceeds this limit, OpenCog logs a warning and injects a truncated
...
message envelopes). If unset, OpenCog uses the host timezone at runtime.
...
`[opencog]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[opencog]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[opencog]"`, OpenCog will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, OpenCog generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.opencog/settings/tts.json",
      elevenlabs: {
...
OpenCog also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, OpenCog currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `opencog configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:opencog`: all built-in OpenCog tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.opencog/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.opencog/sandboxes",
        docker: {
          image: "opencog-sandbox:bookworm-slim",
          containerPrefix: "opencog-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "opencog-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "opencog-sandbox-browser:bookworm-slim",
          containerPrefix: "opencog-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenCog uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.opencog/agents/<agentId>/agent/models.json` or by defining the same schema inside your
OpenCog config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, OpenCog writes/merges a `models.json` into
`~/.opencog/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. OpenCog uses
...
- Shortcut: `opencog onboard --auth-choice opencode-zen`.
...
Shortcut: `opencog onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `opencog onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `opencog onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `opencog onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `opencog onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `opencog onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPENCOG_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.opencog/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.opencog/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.opencog/agents/{agentId}/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, OpenCog stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.opencog/skills` (workspace skills
...
from `~/.opencog/extensions`, `<workspace>/.opencog/extensions`, plus any
...
### `browser` (opencog-managed browser)

OpenCog can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for opencog and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (OpenCog.app menubar, or `opencog gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      opencog: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/opencog" }
...
- Examples: `"/ui"`, `"/opencog"`, `"/apps/opencog"`.
...
- When a connection comes from one of these IPs, OpenCog uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `opencog gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `OPENCOG_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `OPENCOG_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. OpenCog
...
- OpenCog.app watches `~/.opencog/opencog.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.opencog/opencog.json` (or `OPENCOG_CONFIG_PATH`) and applies changes automatically.
...
- `~/.opencog/opencog.json` (or `OPENCOG_CONFIG_PATH`)
...
- `OPENCOG_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `opencog --dev …` → uses `~/.opencog-dev` + shifts ports from base `19001`
- `opencog --profile <name> …` → uses `~/.opencog-<name>` (port via config/env/flags)
...
```bash
OPENCOG_CONFIG_PATH=~/.opencog/a.json \
OPENCOG_STATE_DIR=~/.opencog-a \
opencog gateway --port 19001
```
...
Gmail helper config (used by `opencog webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "opencog@gmail.com",
...
- Set `OPENCOG_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, OpenCog defaults `serve.path` to `/` so
...
Default root: `~/.opencog/workspace/canvas`  
Default port: `18793` (chosen to avoid the opencog browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__opencog__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__opencog__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `OPENCOG_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.opencog/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.opencog/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.opencog/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_opencog-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `opencog` (advertises `opencog.local`). Override with `OPENCOG_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_opencog-gw._tcp` under `~/.opencog/dns/` using the configured discovery domain (example: `opencog.internal.`).
