...
- Replaces the legacy `gateway` command. CLI entry point: `opencog gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
opencog gateway --port 18789
# for full debug/trace logs in stdio:
opencog gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
opencog gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.opencog/opencog.json` (or `OPENCOG_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__opencog__/canvas/` from `~/.opencog/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OPENCOG_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `OPENCOG_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `OPENCOG_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.opencog.*` may still exist)
- Linux: `opencog-gateway-<profile>.service`
- Windows: `OpenCog Gateway (<profile>)`

Install metadata is embedded in the service config:

- `OPENCOG_SERVICE_MARKER=opencog`
- `OPENCOG_SERVICE_KIND=gateway`
- `OPENCOG_SERVICE_VERSION=<version>`
...
```bash
opencog --dev setup
opencog --dev gateway --allow-unconfigured
...
- `OPENCOG_STATE_DIR=~/.opencog-dev`
- `OPENCOG_CONFIG_PATH=~/.opencog-dev/opencog.json`
- `OPENCOG_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.opencog/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `OPENCOG_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `OPENCOG_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `OPENCOG_CONFIG_PATH`
- unique `OPENCOG_STATE_DIR`
...
```bash
opencog --profile main gateway install
opencog --profile rescue gateway install
```

Example:

```bash
OPENCOG_CONFIG_PATH=~/.opencog/a.json OPENCOG_STATE_DIR=~/.opencog-a opencog gateway --port 19001
OPENCOG_CONFIG_PATH=~/.opencog/b.json OPENCOG_STATE_DIR=~/.opencog-b opencog gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `opencog health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `opencog`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `opencog gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.opencog.*` is cleaned up).
  - `opencog doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are OpenCog profile services.
  ...
  - Cleanup: `opencog gateway uninstall` (current service) and `opencog doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `opencog gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- OpenCog.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.opencog.*` labels still unload cleanly).
- To stop it cleanly, use `opencog gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `opencog gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `opencog gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

OpenCog installs a **systemd user service** by default on Linux/WSL2. We
...
`opencog gateway install` writes the user unit. `opencog doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/opencog-gateway[-<profile>].service`:

```
[Unit]
Description=OpenCog Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/opencog gateway --port 18789
Restart=always
RestartSec=5
Environment=OPENCOG_GATEWAY_TOKEN=
...
```
systemctl --user enable --now opencog-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/opencog-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now opencog-gateway[-<profile>].service
```
...
- `opencog gateway health|status` — request health/status over the Gateway WS.
- `opencog message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `opencog agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `opencog gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `opencog gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `opencog gateway` and the legacy TCP control port.
