...
- `/usage cost` in chats: local cost summary aggregated from OpenCog session logs.
- CLI: `opencog status --usage` prints a full per-provider breakdown.
- CLI: `opencog channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
