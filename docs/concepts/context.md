...
read_when:
  - You want to understand what “context” means in OpenCog
...
“Context” is **everything OpenCog sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (OpenCog-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How OpenCog builds the system prompt

The system prompt is **OpenCog-owned** and rebuilt each run. It includes:
...
By default, OpenCog injects a fixed set of workspace files (if present):
