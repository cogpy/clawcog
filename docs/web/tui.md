...
```bash
opencog tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `opencog tui --deliver`
...
- Check the Gateway logs: `opencog logs --follow`.
- Confirm the agent can run: `opencog status` and `opencog models status`.
...
- No agents in picker: check `opencog agents list` and your routing config.
