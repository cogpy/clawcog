...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/opencog`)
...
If the page fails to load, start the Gateway first: `opencog gateway`.
...
you revoke it with `opencog devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.opencog/opencog.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenCog
...
OpenCog **blocks** Control UI connections without device identity.
