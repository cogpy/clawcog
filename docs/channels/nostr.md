...
read_when:
  - You want OpenCog to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OpenCog to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`opencog onboard`) and `opencog channels add` list optional channel plugins.
...
```bash
opencog plugins install @opencog/nostr
```

Use a local checkout (dev workflows):

```bash
opencog plugins install --link <path-to-opencog>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "opencog@example.com",
        "lud16": "opencog@example.com"
      }
    }
  }
}
