...
Matrix is an open, decentralized messaging protocol. OpenCog connects as a Matrix **user**
...
```bash
opencog plugins install @opencog/matrix
```

Local checkout (when running from a git repo):

```bash
opencog plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `opencog plugins install @opencog/matrix`
   - From a local checkout: `opencog plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenCog calls the same
     login endpoint, stores the access token in `~/.opencog/credentials/matrix/credentials.json`,
...
- On first connection, OpenCog requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenCog logs a warning.
...
`~/.opencog/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `opencog pairing list matrix`
  - `opencog pairing approve matrix <CODE>`
...
- On startup, OpenCog resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
