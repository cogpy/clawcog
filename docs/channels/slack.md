...
3. Set tokens for OpenCog and start the gateway.
...
7. Slash Commands → create `/opencog` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
OpenCog can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, OpenCog falls back to the user token.
...
```json
{
  "display_information": {
    "name": "OpenCog",
    "description": "Slack connector for OpenCog"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/opencog",
        "description": "Send a message to OpenCog",
        "should_escape": false
      }
    ]
  },
...
By default, OpenCog replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, OpenCog infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `opencog pairing approve slack <code>`.
...
- On startup, OpenCog resolves channel/user names in allowlists to IDs (when tokens allow)
