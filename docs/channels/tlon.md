...
Tlon is a decentralized messenger built on Urbit. OpenCog connects to your Urbit ship and can
...
```bash
opencog plugins install @opencog/tlon
```

Local checkout (when running from a git repo):

```bash
opencog plugins install ./extensions/tlon
```
...
Use these with `opencog message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenCog replies in-thread.
