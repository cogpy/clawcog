...
```bash
opencog plugins install @opencog/msteams
```

Local checkout (when running from a git repo):

```bash
opencog plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
OpenCog will offer the local install path automatically.
...
3. Configure OpenCog with those credentials.
...
- Talk to OpenCog via Teams DMs, group chats, or channels.
...
- On startup, OpenCog resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.opencog/opencog.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OpenCog, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `opencog-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `opencog plugins install @opencog/msteams`
   - From a local checkout: `opencog plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OpenCog in Teams", "full": "OpenCog in Teams" },
...
OpenCog handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OpenCog only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OpenCogShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

OpenCog sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `opencog message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.opencog/msteams-polls.json`.
...
```bash
opencog message send --channel msteams \
...
opencog message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
opencog message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
opencog message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
opencog message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
