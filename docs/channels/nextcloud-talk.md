...
```bash
opencog plugins install @opencog/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
opencog plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenCog" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `opencog pairing list nextcloud-talk`
  - `opencog pairing approve nextcloud-talk <CODE>`
