...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenCog to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
opencog plugins install @opencog/feishu
```

Local checkout (when running from a git repo):

```bash
opencog plugins install ./extensions/feishu
```
...
If you just installed OpenCog, run the wizard:
...
2. Configuring app credentials in OpenCog
...
1. You already ran `opencog channels add` for Feishu
2. The gateway is running (`opencog gateway status`)
...
```bash
opencog pairing approve feishu <CODE>
```
...
2. Run `opencog logs --follow` and look for `chat_id`
...
2. Run `opencog logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `opencog gateway status`  | Show gateway status           |
| `opencog gateway install` | Install/start gateway service |
| `opencog gateway stop`    | Stop gateway service          |
| `opencog gateway restart` | Restart gateway service       |
| `opencog logs --follow`   | Tail gateway logs             |
...
4. Check logs: `opencog logs --follow`
...
5. Ensure the gateway is running: `opencog gateway status`
6. Check logs: `opencog logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.opencog/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.opencog/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
