...
read_when:
  - Setting up Twitch chat integration for OpenCog
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenCog connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
opencog plugins install @opencog/twitch
```

Local checkout (when running from a git repo):

```bash
opencog plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENCOG_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "opencog", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENCOG_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENCOG_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
