...
- Jobs persist under `~/.opencog/cron/` so restarts don’t lose schedules.
...
opencog cron list
opencog cron run <job-id>
opencog cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.opencog/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `opencog cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, OpenCog defaults to `announce`.
...
- Job store: `~/.opencog/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.opencog/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
opencog cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
opencog cron edit <jobId> --agent ops
opencog cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
opencog cron run <jobId>
opencog cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
opencog cron edit <jobId> \
...
```bash
opencog system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`opencog system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENCOG_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenCog applies exponential retry backoff for recurring jobs after consecutive errors:
