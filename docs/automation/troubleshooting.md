...
opencog system heartbeat last
opencog logs --follow
opencog config get agents.defaults.heartbeat
opencog channels status --probe
```
...
```bash
opencog config get agents.defaults.heartbeat.activeHours
opencog config get agents.defaults.heartbeat.activeHours.timezone
opencog config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
opencog cron list
opencog logs --follow
```
