...
function resolveOpenCogRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENCOG_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENCOG_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenCogRoot(), "dist", "extensionAPI.js");
