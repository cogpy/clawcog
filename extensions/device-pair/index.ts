...
import { approveDevicePairing, listDevicePairing } from "opencog/plugin-sdk";
...
function resolveGatewayPort(cfg: OpenCogPluginApi["config"]): number {
  const envRaw =
    process.env.OPENCOG_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: OpenCogPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: OpenCogPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: OpenCogPluginApi): Promise<ResolveUrlResult> {
