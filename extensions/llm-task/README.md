...
`opencog.invoke --each`) without adding new OpenCog code per workflow.
...
This extension depends on OpenCog internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenCog extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.opencog/extensions` as a standalone plugin directory.
