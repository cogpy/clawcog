import type { OpenCogPluginApi, OpenCogPluginService } from "opencog/plugin-sdk";
...
function normalizeDenyList(cfg: OpenCogPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenCogPluginApi["config"]): string[] {
...
export default function register(api: OpenCogPluginApi) {
  ...
  const timerService: OpenCogPluginService = {
