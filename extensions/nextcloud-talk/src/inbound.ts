...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OpenCogConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OpenCogConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OpenCogConfig);
