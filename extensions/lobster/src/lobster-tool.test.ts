...
import type { OpenCogPluginApi, OpenCogPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "opencog-lobster-plugin-") {
...
function fakeApi(overrides: Partial<OpenCogPluginApi> = {}): OpenCogPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OpenCogPluginToolContext> = {}): OpenCogPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpenCogPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
