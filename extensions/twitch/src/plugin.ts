/**
 * Twitch channel plugin for OpenCog.
 *
...
import { buildChannelConfigSchema } from "opencog/plugin-sdk";
import type {
 ...
 * for OpenCog. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpenCogConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpenCogConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpenCogConfig): boolean => {
