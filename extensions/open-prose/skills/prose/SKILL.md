...
metadata: { "opencog": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenCog `sessions_spawn`
- **File I/O** == OpenCog `read`/`write`
- **Remote fetch** == OpenCog `web_fetch` (or `exec` with curl when POST is required)
