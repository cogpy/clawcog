import type { PluginRuntime } from "opencog/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENCOG_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opencog-nostr-"));
  process.env.OPENCOG_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.OPENCOG_STATE_DIR?.trim() || env.OPENCOG_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".opencog");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENCOG_STATE_DIR = previous;
    }
