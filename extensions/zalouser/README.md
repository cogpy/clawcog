...
OpenCog extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
opencog channels login --channel zalouser
...
```bash
opencog message send --channel zalouser --target <threadId> --message "Hello from OpenCog!"
```
...
```bash
opencog channels login --channel zalouser              # Login via QR
opencog channels login --channel zalouser --account work
opencog channels status --probe
opencog channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
opencog directory self --channel zalouser
opencog directory peers list --channel zalouser --query "name"
opencog directory groups list --channel zalouser --query "work"
opencog directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
opencog message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
opencog message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `opencog channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
opencog channels login --channel zalouser --account work
opencog message send --channel zalouser --account work --target <id> --message "Hello"
