...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opencog-e2e-${name}-`));
  const configDir = path.join(homeDir, ".opencog");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENCOG_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        OPENCOG_GATEWAY_TOKEN: inst.gatewayToken,
        OPENCOG_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPENCOG_GATEWAY_PORT: String(gwA.port),
          OPENCOG_GATEWAY_TOKEN: gwA.gatewayToken,
          OPENCOG_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPENCOG_GATEWAY_PORT: String(gwB.port),
          OPENCOG_GATEWAY_TOKEN: gwB.gatewayToken,
          OPENCOG_GATEWAY_PASSWORD: "",
        }),
