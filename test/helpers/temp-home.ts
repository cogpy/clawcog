...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPENCOG_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.OPENCOG_STATE_DIR = path.join(base, ".opencog");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "opencog-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".opencog", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
