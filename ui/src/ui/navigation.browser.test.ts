...
import { OpenCogApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = OpenCogApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("opencog-app") as OpenCogApp;
...
beforeEach(() => {
  OpenCogApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__OPENCOG_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  OpenCogApp.prototype.connect = originalConnect;
  window.__OPENCOG_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/opencog/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/opencog");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/opencog/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCOG_CONTROL_UI_BASE_PATH__ = "/opencog";
    const app = mountApp("/opencog/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/opencog");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/opencog/sessions");
  });
